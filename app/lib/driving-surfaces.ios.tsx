...
const DriveWidget = createWidget<DriveSurfaceState>('DriveContext', (state) => {
  ...
  return (
    <VStack alignment="leading" spacing={6} modifiers={[padding({ all: 14 })]}>
      <Text modifiers={[font({ size: 12, weight: 'semibold' }), foregroundStyle('#B64032')]}>PIXIE DRIVE CONTEXT</Text>
      <Text modifiers={[font({ size: 19, weight: 'bold' })]}>{state.zone}</Text>
      <Text modifiers={[font({ size: 13 })]}>{state.context}</Text>
      <Text modifiers={[font({ size: 11 }), foregroundStyle('#66777A')]}>{state.factor}</Text>
    </VStack>
  );
});
    ...
    banner: (
      <HStack spacing={8} modifiers={[padding({ all: 14 })]}>
        <VStack alignment="leading" spacing={3}>
          <Text modifiers={[font({ size: 13, weight: 'semibold' }), foregroundStyle('#B64032')]}>Drive context active</Text>
          <Text modifiers={[font({ size: 16, weight: 'bold' })]}>{state.zone}</Text>
          <Text modifiers={[font({ size: 12 })]}>{state.context}</Text>
        </VStack>
        <Spacer />
        <Text modifiers={[font({ size: 11 })]}>{state.factor}</Text>
      </HStack>
    ),
    compactLeading: compact,
    compactTrailing: <Text modifiers={[font({ size: 11 })]}>LIVE</Text>,
    minimal: compact,
    expandedCenter: <Text modifiers={[font({ size: 14, weight: 'semibold' })]}>{state.zone}: {state.context}</Text>,
  };
});
