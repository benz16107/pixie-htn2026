...
export function Receipt({ r }: { r: R }) {
  ...
  return (
    <section aria-labelledby="rc" className="max-w-[560px]">
      ...
      <table className="w-full border-collapse font-mono text-[12px]">
        <tbody>
          <tr className="border-t border-ink">
            <th scope="row" className="py-1.5 text-left font-sans font-medium">Base rate</th>
            <td />
          ...
          {r.lines.map((l) => (
            <tr key={l.label} className="border-t border-dashed border-rule align-top" title={l.source}>
              ...
              <td className="num w-[90px] py-1.5 text-right text-dim">
                ...
                {l.capped && <span className="ml-1 text-[10px] text-ink">[cap]</span>}
              </td>
              <td className="num w-[90px] py-1.5 text-right">{l.dollars > 0 ? "+" : ""}{usd(l.dollars)}</td>
            </tr>
          ))}
          <tr className="border-t-2 border-ink text-[14px]">
            <th scope="row" className="py-2 text-left font-sans font-semibold">Annual</th>
      ...
      <p className="mt-1.5 text-[11.5px] text-dim">{r.label}</p>
    </section>
  );
}
