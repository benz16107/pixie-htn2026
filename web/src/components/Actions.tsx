...
export function Actions({ caseId, facts, proposed }: { caseId: string; facts: string[]; proposed?: string }) {
  ...
  return (
    <div className="flex shrink-0 flex-col items-end gap-2">
      ...
      <span role="status" className="num text-[12px] text-dim">
        {status.req ? `broker: ${status.req}` : proposed === "proposed" ? "the desk proposed a broker request" : proposed ? `broker request ${proposed}` : ""}
