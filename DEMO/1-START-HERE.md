...
2. For commercial tracks, open `/queue`, case 138, and the screen named on your track card. For renter tracks, open `/intact`, `/intact/quotes`, and the Expo app. Keep `/backtest` ready for questions.
...
4. Treat `/live` as an optional OpenAI orchestration screen. It is not part of the default Federato demo. If a judge asks how the agents coordinate, open it directly and use the recorded case run.
...
| 2:20-3:20 | Case 138, evidence and decision trail | "The specialists attach findings to the case. The Challenger records objections, and the final decision keeps the evidence and resolution together." |
