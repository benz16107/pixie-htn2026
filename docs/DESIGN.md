...
1. **The queue that matters is small and mostly out of appetite.** 21 open submissions, 6 of them property (126, 133, 134, 138, 141, 143). Five are in TX, IL or WA, outside the 2025 state list. Only 138 (FL, 2023 masonry non-combustible, $2.1M TIV) looks in appetite. A desk that just prints "decline" six times has no story. The story is *what the desk had to find out to be sure*.
2. **Open submissions have no premium, no business type and no building link.** TIV comes from `Insured.hq -> Location.buildings`. Premium must be estimated. New vs renewal must be inferred from the insured's other policies (143's insured has since bound property coverage, PR-2026-1031 quoted 2025-11-25, after 143 was received on 2025-08-19, so 143 looks stale or duplicate: ask the broker). Loss history must come from the insured's prior policies' claims.
...
a.decision         # Decided(kind="decline", because=["state:IL not_acceptable", "age:1950 not_acceptable"])
...
Two runners designed independently from the same grounding: candidate-1 (Opus, "the interval desk") and candidate-2 (Fable, "the case file is the trace"). Both converged on one event log per case that serves as trace, UI feed, replay, and audit. A Fable cross-judge scored candidate-1 higher on Federato fit, multi-agent quality, track honesty, and demo (all 5), and candidate-2 higher on the Intact slide-over (5 vs 4) and solo buildability (4 vs 3). The judge also checked facts against the data.

**Base: candidate-1.** Its types carry the invariants: `Value = Known | Estimated | Missing` means no rule can read a bare float, and an estimate widens the interval instead of deciding. `Open` with `Flipper`s makes Federato's "adapts, deepens for high-value" a computable policy. `assess()` is one function for both audiences, and the public API is smaller (10 names).

**Grafted from candidate-2** (see `arena/candidate-2/sketch/` for the source shapes):
1. **SQLite `CaseStore`** (`candidate-2/sketch/case.py`): one file, `var/atlas.sqlite`, holding events, cases, the lookup cache and the outbox. It replaces candidate-1's JSONL-per-case, tenant JSON files, outbox JSONL and layer cache. `tail(after=seq)` is the SSE cursor, and the file is copied at freeze as `var/atlas-demo.sqlite`.
2. **Two named hydration plans** (`candidate-2/sketch/federato.py` `HYDRATION_PLANS`): bound via `Policy.exposure_units -> Location -> buildings`, open via `Insured.hq -> buildings`. Intake picks between them and says why. Keep candidate-1's `SchemaGraph.paths()` for the TIV-path demo beat and `QueryBuilder.lint`; general BFS over reverse references becomes a stretch goal.
3. **`depth_policy` floor** (`candidate-2/sketch/desk.py`): code proposes a depth (skim, standard, or deep) from value and preliminary factors, and the Lead may raise it but never lower it. This is the fallback when too many intervals stay `Open`.
4. **Lane order for Intact:** Codex builds the Expo app from 23:00 against mock JSON, not the web agent at 00:15. The Intact side also gets `/privacy` and `/terms` pages and an exact-sum receipt test.
5. **`REASON_TO_LANE`** (`candidate-2/sketch/backtest.py`) for the declines backtest. Each human `decline_reason` maps to the lane that should raise it, including an attempt on `insufficient_controls` (controls gaps in ExposureUnit data) instead of marking it `not_modeled`.

**Rejected from candidate-2:** the estimated premium triggering a hard fail (`unknown_when_estimated: false`), which lets an estimate decide a decline; the `dict` escape hatch in `Payload`; the live "Run" press as the default demo (replay is the default); and placeholder incident entries.

**Fixed from the judge's fact check:**
- Five of the six open property submissions are outside the appetite states (126 TX, 133 IL, 134 TX, 141 TX, 143 WA). Only 138 (FL) is in-list.
- 133's TIV-weighted build year is 1950.4.
- 143's "active policy" was quoted after 143 was received, so the story is "stale or duplicate, ask the broker", not "renewal disguised as new".
- The whole open queue targets October 2025, so no "this morning's queue" copy.
- ExposureUnit, Contact, Coverage and Endorsement are now pulled (938, 14, 366 and 253 records) and sit in `data/federato/`.

## Amendments from the "both miss" review

- **Scope trims:**
  - Sentry is one Python project (agent spans, tracing, Logs) plus Expo JS errors. Session Replay, uptime and cron monitors are COULD.
  - The backtest reports plain counts and rates with n, with no Spearman or Kendall.
  - The web hex map is Tier 2 with a 90-minute timebox; the portfolio line on the case page carries the point if it's cut.
  - The Toronto pack starts with three datasets (break-ins, basement flooding, fire stations), with the floodline and hydrants as extras.
- **LLM budget:** specialists run on the cheaper configured model and the Lead on the flagship. The precompute (about 8 deep-dive cases x about 12 calls) runs at 01:30 with a hard cost cap in config, before sleep, never during it.
- **Federato rate limits are unknown,** so every deterministic path reads the local snapshot (all 12 resources pulled). Live Federato calls happen only for Intake's queries in the ask box and the live-mode case.
- **Intact criterion 5** ("substantially the team's own work"): the README states plainly that one person built it with AI coding agents, and links `CODEX.md`.
