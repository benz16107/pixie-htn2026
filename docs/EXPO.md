...
The main sequence is address, map, three questions, and quote. The result screen shows the annual price, an itemized source-labelled receipt, the decision, and the next step. The app can read the result with on-device speech, create a PDF with `expo-print`, share it, and open the referred case in the web desk.
