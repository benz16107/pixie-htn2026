...
| OpenAI | `desk.py` gives typed tools to specialist agents and verifies numerical claims | Commercial replay uses recorded events without a model call |
...
| Backboard | `memory.py` stores underwriter context and returns explicitly labelled model judgments | Cached responses remain labelled with their source |
